...
def current_version() -> str:
    ...
    if manifest.exists():                                    # the two plugins
    ...
    ext = sorted(HERE.glob("_extensions/*/_extension.yml"))
    if ext:                                                  # the theme
        m = re.search(r"^version:\s*(\S+)", ext[0].read_text(), re.M)
        if m:
            return m.group(1)

    sys.exit("no version found: expected manifest.json, src/qdapy/_version.py, "
             "DESCRIPTION or _extensions/*/_extension.yml in the working "
             "directory")
